...
IA::Qtuple_info &IA::find_nb_qtuples_vertical(
    ...
    for (int y = posY + currI; y + currI != posY + 5 && currI <= 0; ++y, ++i) {
        ...
        if ((y < 0 || y >= MAP_SIZE) || (i == 0 && ((y >= 0 && y < MAP_SIZE && map[y][posX] == playerSymbols[1])
                       || (y + 1 >= 0 && y + 1 < MAP_SIZE && map[y + 1][posX] == playerSymbols[1])
                       || (y + 2 >= 0 && y + 2 < MAP_SIZE && map[y + 2][posX] == playerSymbols[1])
                       || (y + 3 >= 0 && y + 3 < MAP_SIZE && map[y + 3][posX] == playerSymbols[1])
                       || (y + 4 >= 0 && y + 4 < MAP_SIZE && map[y + 4][posX] == playerSymbols[1])))) {
